...
void VolumeIntegral::compProjectionIntegrals(PolyhedronFace *f)
{
  double a0, a1, da;
  double b0, b1, db;
...
void VolumeIntegral::compNormals(Polyhedron* p)
{
	for (int i = 0; i < p->numFaces; i++) {
	  PolyhedronFace* f = &p->faces[i];
	  f->poly = p;
	  /* compute face normal and offset w from first 3 vertices */
...
void VolumeIntegral::compFaceIntegrals(PolyhedronFace *f)
{
  const double *n;
  double w;
  double k1, k2, k3, k4;
...
void VolumeIntegral::compVolumeIntegrals(Polyhedron *p)
{
  PolyhedronFace *f;
  double nx, ny, nz;
  int i;
...
void VolumeIntegral::compute(Polyhedron& p, double J[3][3], double r[3], double& volume)
{
  double density;
