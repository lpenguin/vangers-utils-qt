...
bool _decodeRaw(
	...
	for(int iRow = 0; iRow < sizeY; iRow++){
		...
		if(!reader.tryReadArray(dataHeight, sizeX)){
			return false;
		}
		if(!reader.tryReadArray(dataMeta, sizeX)){
			return false;
		}
...
bool _readPalette(Palette & palette, const QDir& fileDir, const QSettings& settings){
	...
	palette << Palette::read(paletteFile);
	paletteFile.close();
	return true;
}
