...
ImageViewer::ImageViewer(ImageViewerPlugin* plugin, QWidget *parent) :
    ...
    _image(nullptr),
	_palette(Palette::grayscale()),
    _accesses({
				 {ImageViewerPlugin::vBmpType.name, QSharedPointer<BmpImageAccess>::create()},
				 {ImageViewerPlugin::BmpType.name, QSharedPointer<QtImageAccess>::create("bmp")},
				 {ImageViewerPlugin::XbmType.name, QSharedPointer<XbmImageAccess>::create()},
				 {ImageViewerPlugin::PngType.name, QSharedPointer<QtImageAccess>::create("png")},
				 {ImageViewerPlugin::TiffType.name, QSharedPointer<QtImageAccess>::create("tiff")},
				 {ImageViewerPlugin::SvsType.name, QSharedPointer<SvsImageAccess>::create()},
				 {ImageViewerPlugin::MssType.name, QSharedPointer<MssImageAccess>::create()},
                 }),
    ...
    _settings("lpenguin", "Vangers Resource Explorer")
{
	...
	_palette = Palette::read(ui->paletteViewer->currentPalette());
}
...
bool ImageViewer::tryRead(Image& image, const QString& fileName, const ResourceType& resourceType){
    QFile f(fileName);
...
{
	_image = QSharedPointer<Image>::create();
...
void ImageViewer::setPalette(const Palette palette)
{
    _palette = palette;
    updateImage();
}
...
{
	_palette = Palette::read(paletteName);
    updateImage();
}
...
//    ui->inageTypeLabel->setText(_image->typeName());
    ...
    if(!_image->hasEmbeddedPalette()){
		Palette palette = Palette(_palette);
	...
	for(auto& field: meta.keys()){
		...
		layout->addRow(toString(field), label);
    }
