...
vangers::Palette vangers::Palette::fire()
{
...
vangers::Palette vangers::Palette::fireball(const vangers::Palette& basePalette, unsigned char baseColor)
{
    unsigned char FireColorTable[256 << 8];
    memset(FireColorTable, 0, 256 << 8);

    FireTableInit(FireColorTable);
    vangers::Palette result;
    for(int i = 0; i < 256; i++){
