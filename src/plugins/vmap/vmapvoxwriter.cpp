...
#include <plugins/vmap/layer/heightimagelayer.h>
#include <plugins/vmap/layer/terrainimagelayer.h>
#include <plugins/vmap/layer/doublelevelimagelayer.h>
...
void vangers::VmapVoxWriter::write(const Vmap& vmap, QString filename)
{
	...
	vangers::Palette palette = vangers::Palette::grayscale();
	TerrainImageLayer terrainLayer(nullptr);
	const Matrix<uint8_t>& height = vmap.heightConst();
	const Matrix<uint8_t>& meta = vmap.metaConst();

	vangers::Palette terrainPalette = terrainLayer.getPalette(vmap);
